    ...
    */
    ...
    + (throwIf (hasAttrByPath ["gui" "apiKey"] cfg.settings) ''
      ...
      '' (lib.optionalString (cfg.apiKeyFile != null) ''
        # `set -e` only takes effect if we use a separate variable
        new_apiKey={\"apiKey\":\"$(cat ${lib.escapeShellArg cfg.apiKeyFile})\"}
        curl -X PUT -d "$new_apiKey" ${curlAddressArgs "/rest/config/gui"}
      ''))
    + ''
    ...
    services.syncthing = {
      ...
      apiKeyFile = mkOption {
        ...
        description = ''
          ...
          Use this instead of
          [`settings.gui.apiKey`](#opt-services.syncthing.settings.gui.apiKey)
