{
  ...
  options = {
    chown-paths = lib.mkOption {
      default = lib.map (path: {inherit path user group;}) pathsToChown;
      description = "A list of paths to recursively chown for user:group.";
      type = with lib.types; listOf (attrsOf str); # FIXME model this better
    };
  };
  config = {
